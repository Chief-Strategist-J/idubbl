...
export const GAME_META = {
  ...
  lucky_wheel: {
    subtitle: 'Spin & Win',
    description: 'Predict which sector the fortune wheel will land on before it spins. The player whose prediction is closest to the actual result wins the round.',
    difficulty: 'Easy',
    color: '#8b5cf6',
    emoji: '🎡',
    imageUrl: 'https://images.unsplash.com/photo-1596838132731-3301c3fd4317?auto=format&fit=crop&w=300&q=80',
    howToPlay: [
      'Before the spin, both players independently select a sector on the wheel (e.g., Red, Blue, Gold, Silver).',
      'Both predictions must be locked in before the countdown ends — after that, no changes are allowed.',
      'The wheel is spun using a server-side Provably Fair algorithm (seeded RNG).',
      'The player whose chosen sector matches or is closest to where the wheel lands wins the round.',
      'Best of 3 rounds wins the match.',
    ],
    rules: [
      'The wheel outcome is determined server-side before the visual spin animation begins — the animation is purely cosmetic.',
      'If both players pick the same sector and it wins, the tiebreaker is a separate server-generated random coin flip.',
      'Failing to lock a prediction before the timer expires results in a random sector being assigned automatically.',
      'Sector probabilities are equal — all sectors have the same likelihood of being the result.',
      'The provably fair seed is revealed after each round so players can verify the fairness of results.',
    ],
  },

  lucky_balls: {
    subtitle: 'Lotto Draw',
    description: 'Pick 3 lucky numbers before the draw. The server draws 5 balls from 1–20. The player who matches more of their numbers to the drawn balls wins the round.',
    difficulty: 'Easy',
    color: '#f97316',
    emoji: '🔮',
    imageUrl: 'https://images.unsplash.com/photo-1518895949257-7621c3c786d7?auto=format&fit=crop&w=300&q=80',
    howToPlay: [
      'Before each round, select 3 unique numbers from the range 1 to 20.',
      'Lock in your picks before the countdown expires.',
      'The server draws 5 random balls (without replacement) from 1–20.',
      'Count how many of your 3 numbers appear among the 5 drawn balls.',
      'The player with more matches wins that round. Best of 3 wins the match.',
    ],
    rules: [
      'All 3 of your picks must be unique — duplicate picks are not allowed.',
      'If you match all 3 of your numbers to the drawn balls, you win the round instantly, regardless of opponent\'s result.',
      'If both players match the same number of balls, the round is a tie — neither wins.',
      'Draws are performed server-side with a cryptographically secure RNG, verified as Provably Fair.',
      'Picks not submitted before the timer expires result in 3 randomly assigned numbers.',
    ],
  },
