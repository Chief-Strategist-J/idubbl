...
export default function TierCard({ tier }) {
  ...
  return (
    <div className={`glass-card tier-card ${tier.color}`} style={{ position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '1.5rem 0', padding: '0.75rem', background: 'var(--glass-bg)', borderRadius: 8 }}>
        <div>
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <p style={{ fontWeight: 600, color: tier.waitingCount > 0 ? 'var(--accent-green)' : 'var(--accent-warning)', fontSize: '0.95rem' }}>{waitLabel}</p>
        </div>
      </div>
